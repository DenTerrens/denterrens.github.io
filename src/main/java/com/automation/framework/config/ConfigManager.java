...
public final class ConfigManager {
    ...
    private static Properties loadProperties() {
        ...
        loadInto(properties, "config/environments/" + System.getProperty("env", "local") + ".properties");
        return properties;
    }
    ...
    private static void loadInto(Properties target, String resourcePath) {
        try (InputStream stream = Thread.currentThread().getContextClassLoader().getResourceAsStream(resourcePath)) {
            ...
            throw new UncheckedIOException("Unable to load configuration resource: " + resourcePath, exception);
        }
    }

    public static String get(String key) {
        return System.getProperty(key, Objects.requireNonNull(PROPERTIES.getProperty(key), () -> "Missing configuration key: " + key));
    }
